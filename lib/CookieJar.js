...
class CookieJar {
	...
	setCookie(cookie, value = undefined, options = {}) {
		...
		if(typeof value !== "undefined") {
			...
			for(var [i, key, _value] of iterate(options)) {
				if(_value == true) _cookie.flags.push(key);
				else if(_value == false) _cookie.flags.splice(_cookie.flags.indexOf(key), 1);
				else _cookie.props[CookieJar.Cookie.formatKeyword(key) || key] = _value;
			}
		...
		if(typeof cookie == "object") {
			...
			var jsonObject = "cookies" in cookie ? cookie.cookies : null;
		...
		return cookieStringArray.map(cookieString => {
			...
			for(var property of properties) {
				...
				if(key) {
					...
					console.warn("Failed to parse cookie: '" + property + "'");
				}
			}
